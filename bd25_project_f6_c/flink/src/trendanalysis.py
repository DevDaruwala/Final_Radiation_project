...
def analyze(record):
    try:
        ...
        return json.dumps({"event_id": cleaned.get("event_id"),
...
def trendjob():
    ...
    analyzed_stream = (data_stream.map(analyze, output_type=Types.STRING()).filter(lambda x: x is not None))
    analyzed_stream.print()

    kafka_sink = FlinkKafkaProducer(topic='processed-data',serialization_schema=SimpleStringSchema(),producer_config={'bootstrap.servers': 'kafka:9092'})
